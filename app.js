...
const startPieces = [
  //creates and sets positioning of the game pieces
  rook,
  knight,
  bishop,
  queen,
...
function createBoard() {
  startPieces.forEach((startPiece, i) => {
    ...
    square.classList.add("square"); //adds a class to that new div
    square.innerHTML = startPiece; //puts the pieces on the board through updating the divs innerHTML
    square.firstChild?.setAttribute("draggable", true); // if div has a first child, set an attribute draggable to true
    square.setAttribute("square-id", i); //sets a new attribute for the new div
    //square.classList.add('beige')
    const row = Math.floor((63 - i) / 8) + 1; //identifies the current row

    if (row % 2 === 0) {
      square.classList.add(i % 2 === 0 ? "beige" : "brown"); //selects even rows and alternates color of squares
    } else {
      square.classList.add(i % 2 === 0 ? "brown" : "beige"); // does the same for the uneven rows
    }

    if (i <= 15) {
      square.firstChild.firstChild.classList.add("Black"); //sets the color of the top side pieces to black
    }

    if (i >= 48) {
      square.firstChild.firstChild.classList.add("White"); //sets the color of the top side pieces to white
    }

    gameBoard.append(square); //adds the squares to the gameboard
  });
}
...
function dragDrop(e) {
  ...
  if (correctGo) {
    //must check this first because if the piece is taken by the opponent, it will be removed from the board
